...
export default function ProjectMetrics() {
  ...
  // Get project data from location state or session storage
  ...
  useEffect(() => {
    if (location.state?.projectData) {
      ...
      // Try to get from session storage or saved projects
      const savedProjects = JSON.parse(localStorage.getItem('savedProjects') || '[]')
      const project = savedProjects.find((p: any) => p.name === projectName)
      if (project) {
        setProjectData({
          id: project.id,
          name: project.name,
  ...
  // Create usage history with the current project as the most recent entry
  const usageHistory = projectData ? [
    {
      id: projectData.id,
  ...
  return (
    <MainLayout
      title={`Metrics for ${projectName}`}
      description="Detailed analytics and performance insights for this project"
      actions={
              ...
              <Card className="bg-gradient-card border-border">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Records Extracted</p>
                      <p className="text-3xl font-bold text-foreground">{projectData.totalRecords.toLocaleString()}</p>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-success">{projectData.accuracy}%</p>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-foreground">{projectData.completionTime}</p>
                      <p className="text-xs text-info mt-1">Efficient</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-foreground">${projectData.totalCost}</p>
                      <p className="text-xs text-accent mt-1">Cost efficient</p>
                    </div>
                ...
                <div key={index} className="p-4 bg-background rounded-lg border border-border">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-foreground">{job.accuracy}%</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-foreground">${job.totalCost}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="bg-gradient-card border-border">
            <CardHeader>
              ...
              <CardDescription>Key insights for this project</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-success/10 border border-success/20 rounded-lg">
                <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center justify-between p-3 bg-info/10 border border-info/20 rounded-lg">
                ...
                <span className="text-sm font-bold text-info">Optimal</span>
              </div>
